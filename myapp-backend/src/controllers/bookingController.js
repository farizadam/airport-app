...
class BookingController {
  ...
  static async create(req, res, next) {
    try {
      ...
      const { seats, pickup_location, dropoff_location } = req.validatedBody || req.body; // Fallback to req.body if validatedBody doesn't have them yet
    ...
    try {
      ...
      if (status && status !== oldStatus) {
        ...
        if (oldStatus === "pending" && status === "accepted") {
          ...
          console.log(`[BookingController] Decremented seats_left for ride ${rideId} by ${booking.seats}`);
          ...
          console.log(`[BookingController] Incremented seats_left for ride ${rideId} by ${booking.seats}`);
        }
      }
      ...
      if (status && status !== oldStatus) {
        if (status === "accepted") {
          // const driver = await User.findById(booking.ride_id.driver_id); // Driver is populated in ride.driver_id
