...
const mapDirection = (
  dir: "to_airport" | "from_airport",
): "home_to_airport" | "airport_to_home" => {
...
export const useRideStore = create<RideState>((set) => ({
  ...
  createRide: async (data) => {
    ...
    try {
      ...
      console.error(
        "❌ Create ride error - Full response:",
  ...
  updateRide: async (id, data) => {
    try {
      ...
      if (data.departure_datetime)
        backendData.datetime_start = data.departure_datetime;
      if (data.home_address !== undefined)
        backendData.home_address = data.home_address;
      ...
      if (data.luggage_capacity !== undefined)
        backendData.luggage_capacity = data.luggage_capacity;
      if (data.driver_comment !== undefined)
        backendData.comment = data.driver_comment;

      const response = await api.patch<{ data: Ride }>(
        `/rides/${id}`,
        backendData,
      );
      set((state) => ({
  ...
  cancelRide: async (id) => {
    try {
      ...
      // Refresh wallet to show any refunds from cancelled bookings
      try {
        const walletStore = useWalletStore.getState();
        await walletStore.getWallet();
        console.log("✅ Wallet refreshed after ride cancellation");
      } catch (walletError) {
        console.warn(
          "⚠️ Failed to refresh wallet after ride cancellation:",
          walletError,
        );
      }
    } catch (error: any) {
