...
export default function RequestDetailsScreen() {
  ...
  // @ts-ignore
  const { fetchRequestById, cancelRequest, makeOffer, acceptOffer, rejectOffer, currentRequest } = useRequestStore();
  ...
  useEffect(() => {
    ...
    logTripScreenOpened("request", id, "RequestDetailsScreen");
  ...
  useEffect(() => {
    ...
    logTripScreenDisplayed("request", id, "RequestDetailsScreen");
    setDisplayedLoggedForId(id);
  }, [id, loading, request, displayedLoggedForId]);
    ...
    useCallback(() => {
      const loadData = async () => {
        if (id) {
          try {
            let seeded = false;
            const cachedId = (currentRequest as any)?._id || (currentRequest as any)?.id;
            const isSameRequest = cachedId && String(cachedId) === String(id);
            if (isSameRequest && !request) {
              setRequest(currentRequest as RideRequest);
              setLoading(false);
              seeded = true;
            }
            ...
            if (!seeded && !request) {
              ...
              const fromLists =
                [...(storeState.requests || []), ...(storeState.myOffers || [])].find(
                  (r: any) => String(r?._id || r?.id) === String(id)
                ) || null;
              if (fromLists) {
                setRequest(fromLists as RideRequest);
                setLoading(false);
                seeded = true;
              }
            }

            if (!seeded && !request && !isSameRequest) {
              setLoading(true);
            }
      ...
      return () => {
        // Keep previous data to avoid blank flash on quick back/forth navigation.
      };
    }, [id, currentRequest, request])
  );
  ...
  const luggageItems = request.luggage?.filter((item) => item.quantity > 0) ?? [];
  const totalLuggage = luggageItems.reduce(
    (sum, item) => sum + (item.quantity || 0),
    0,
  );
  const luggageBreakdown = (() => {
    if (!totalLuggage) return "";
    const getQty = (type: string) =>
      luggageItems.find((item) => item.type === type)?.quantity || 0;
    const qty10 = getQty("10kg");
    const qty20 = getQty("20kg");
    const qtySac = getQty("sac");
    const qtyHors = getQty("hors_norme");
            ...
            <View style={styles.mapContainer}>
              ...
              <View style={styles.mapLegend}>
                <View style={styles.legendItem}>
                  <View style={[styles.legendDot, { backgroundColor: request.direction === 'to_airport' ? "#EF4444" : "#007AFF" }]} />
                  <Text style={styles.legendText}>
                    {request.direction === 'to_airport' ? 'Pickup' : 'Airport'}
                  </Text>
                </View>
                <View style={styles.legendItem}>
                  <View style={[styles.legendDot, { backgroundColor: request.direction === 'to_airport' ? "#007AFF" : "#EF4444" }]} />
                  <Text style={styles.legendText}>
          ...
          <View style={styles.detailsGrid}>
            ...
            <View style={styles.detailItem}>
              ...
              <Text style={styles.detailValue}>
                {totalLuggage > 0
                  ? `${totalLuggage} bag${totalLuggage === 1 ? "" : "s"}`
                  : "None"}
              </Text>
            ...
            return (
              <View style={styles.section}>
                ...
                <View style={styles.contactButtonsRow}>
                  ...
                  >
                    <Ionicons name="chatbubbles" size={20} color="#fff" style={styles.chatContactBtnIcon} />
