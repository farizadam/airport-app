...
export default function MyTripsScreen() {
  ...
  // First-login race guard: run one forced refresh shortly after auth flips true.
  useEffect(() => {
    if (!isAuthenticated) return;
    if (hasAnyTripsData) return;

    const t = setTimeout(() => {
      loadData({ silent: true, forceFresh: true });
    }, 1200);

    return () => clearTimeout(t);
  }, [isAuthenticated, hasAnyTripsData]);
  ...
  useFocusEffect(
    useCallback(() => {
      if (isAuthenticated) {
        ...
        loadData({ silent: true, forceFresh: shouldForce });
      }
    }, [isAuthenticated, hasAnyTripsData])
  );

  const loadData = async (options?: { silent?: boolean; forceFresh?: boolean }) => {
    ...
    if (forceFresh) {
      clearApiGetCache("/rides/my-rides");
      clearApiGetCache("/ride-requests/my-requests");
      clearApiGetCache("/ride-requests/my-offers");
      clearApiGetCache("/bookings/my-bookings");
    }
    ...
    const request = (async () => {
      ...
      // Fetch bookings in background; don't block initial trips rendering.
      getMyBookings({ force: forceFresh }).catch(() => { });
    })();
  ...
  const isLoading =
    (screenLoading && !hasAnyTripsData) ||
    ((ridesLoading || requestsLoading || bookingsLoading) && !lastLoadedAtRef.current && !hasAnyTripsData);
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingVertical: 16, // Adjusted vertical padding
  },
  headerTitle: {
    fontSize: 28,
    fontWeight: "800",
