...
export default function RideDetailsScreen() {
  ...
  // Function to fetch ride data (used for refresh and after actions)
  ...
  const luggageSummary = useMemo(() => {
    const capacity = ride?.luggage_capacity as
      | number
    ...
    const remaining =
      typeof legacyLeft === "number"
        ? legacyLeft
        : ride?.luggage_remaining
          ? Object.values(ride.luggage_remaining).reduce(
              (sum, value) => sum + (Number(value) || 0),
              0,
            )
          : 0;
    ...
    return {
      totalCapacity,
      used: Math.max(0, totalCapacity - remaining),
    };
  }, [ride]);
  const luggageBreakdown = useMemo(() => {
    const capacity = ride?.luggage_capacity as
      | number
      | Record<string, number>
      | undefined;
    const remaining = ride?.luggage_remaining as Record<string, number> | undefined;
  ...
  const rideDate = ride.datetime_start
    ...
    : new Date();
  const renderLuggageItem = (apiType: string, label: string, capacityKey: string) => {
    const available = ((ride.luggage_remaining as any)?.[capacityKey] as number) ?? 0;
    const current = luggage.find((item) => item.type === apiType)?.quantity ?? 0;
    const setQty = (qty: number) => {
      setLuggage((prev) => {
        const filtered = prev.filter((item) => item.type !== apiType);
        return qty > 0 ? [...filtered, { type: apiType, quantity: qty }] : filtered;
      });
    };

    return (
      <View style={styles.luggageGridItem}>
        <View style={styles.luggageGridHeader}>
          <Text style={styles.luggageGridLabel}>{label}</Text>
          <Text style={[styles.luggageGridAvailability, { color: available > 0 ? "#64748B" : "#EF4444" }]}>
            {`${current}/${available}`}
          </Text>
        </View>
        <View style={styles.luggageStepperContainer}>
          <TouchableOpacity
            style={[styles.luggageStepperButton, current <= 0 && styles.stepperButtonDisabled]}
            onPress={() => setQty(Math.max(0, current - 1))}
            disabled={current <= 0}
          >
            <Ionicons name="remove" size={16} color={current <= 0 ? "#CBD5E1" : "#1E293B"} />
          </TouchableOpacity>
          <Text style={styles.luggageStepperValue}>{current}</Text>
          <TouchableOpacity
            style={[styles.luggageStepperButton, (current >= available || available === 0) && styles.stepperButtonDisabled]}
          ...
          >
            <Ionicons
              name="add"
              size={16}
              color={(current >= available || available === 0) ? "#CBD5E1" : "#1E293B"}
            />
          </TouchableOpacity>
        </View>
      </View>
    ...
    <SafeAreaView style={styles.container} edges={["top"]} key={id}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.detailsGrid}>
            ...
            <View style={styles.detailItem}>
              ...
              <Text style={styles.detailValue}>
                {luggageSummary.totalCapacity > 0
                  ? `${luggageSummary.used} / ${luggageSummary.totalCapacity}`
                  : "0 / 0"}
              </Text>
            ...
            >
              <ProfileAvatar
                ...
                lastName={ride.driver?.last_name}
      ...
      >
        <View style={styles.modalContainer}>
          <View style={[styles.modalContent, { marginBottom: insets.bottom + 8 }]}> 
            ...
            <View style={styles.luggageGridRow}>
              {renderLuggageItem("sac", "🎒 Cabin Bag", "count_sac")}
              {renderLuggageItem("10kg", "🧳 Small (10kg)", "count_10kg")}
            </View>
            <View style={styles.luggageGridRow}>
              {renderLuggageItem("20kg", "💼 Large (20kg)", "count_20kg")}
              {renderLuggageItem("hors_norme", "📦 Oversized", "count_hors_norme")}
            </View>
            ...
            <View style={[styles.modalActions, { paddingBottom: Math.max(8, insets.bottom) }]}>
              <TouchableOpacity
