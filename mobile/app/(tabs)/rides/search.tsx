...
const getLuggageCapacityTotal = (capacity: unknown) => {
...
const getLuggageRemainingTotal = (ride: any) => {
  if (typeof ride?.luggage_left === "number") return ride.luggage_left;
...
export default function SearchScreen() {
        ...
        try {
          ...
          if (Array.isArray(ridesData)) {
            ridesData.forEach((ride: any) => {
              const luggageCapacityTotal = getLuggageCapacityTotal(ride.luggage_capacity);
              const luggageRemainingTotal = getLuggageRemainingTotal(ride);
              if (ride.driver) {
              ...
              results.push({
                ...
                luggageCapacity: luggageCapacityTotal,
                luggage_left: typeof luggageRemainingTotal === "number" ? luggageRemainingTotal : undefined,
  ...
  const filteredAirports = airports.filter((airport: Airport) => {
    ...
    const iata = (airport?.iata_code || (airport as any)?.code || "").toLowerCase();
  ...
  const renderAirportStep = () => (
    <FlatList
      ...
      renderItem={({ item }) => (
        ...
        >
          <View style={[styles.airportIconCircle, { backgroundColor: themeColor }]}> 
            <Ionicons name="airplane" size={20} color="#fff" />
          </View>
          {!!(item.iata_code || (item as any)?.code) && (
            <View style={[styles.airportCodeBadge, { backgroundColor: themeColor + "20" }]}> 
              <Text style={[styles.airportCode, { color: themeColor }]}> 
                {(item.iata_code || (item as any)?.code).toUpperCase()}
              </Text>
            </View>
          )}
    ...
    <View style={styles.stepContent}>
      ...
      <View style={styles.dateTimeSection}>
        ...
        <Text style={styles.timeNote}>
          Leaving time optional will show rides within +/-4 hours of the selected day.
        </Text>
      </View>
          ...
          return (
            <View style={styles.resultsList}>
              {filteredResults.map((result) => {
                ...
                const luggageCapacityTotal = getLuggageCapacityTotal(result.luggageCapacity);
                const luggageRemainingTotal =
                  typeof result.luggage_left === "number" ? result.luggage_left : undefined;
                const luggageText = isRide
                  ? luggageCapacityTotal > 0
                    ? typeof luggageRemainingTotal === "number"
                      ? `${Math.max(0, luggageCapacityTotal - luggageRemainingTotal)}/${luggageCapacityTotal} bags`
                      : `${luggageCapacityTotal} bags`
                    : "0 bags"
