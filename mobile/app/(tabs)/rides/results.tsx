...
interface SearchResult {
  ...
  luggageCapacityTotal?: number;
  luggageRemainingTotal?: number;
  luggage_left?: number;
  luggageCount?: number;
  passengers?: number;
  pricePerSeat?: number;
...
export default function SearchResultsScreen() {
  ...
  const processResults = () => {
    ...
    rides?.forEach((ride: any) => {
      ...
      const totalSeats = ride.seats_total || ride.total_seats || ride.seats_left || ride.available_seats || 0;
      const luggageCapacityTotal = (() => {
        const capacity = ride.luggage_capacity;
      ...
      const luggageRemainingTotal = (() => {
        const legacyLeft = ride.luggage_left;
      ...
      if (dateMatch && seatsMatch) {
        ...
        mockResults.push({
          ...
          luggageCapacityTotal,
          luggageRemainingTotal,
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8 }}>
            <View style={styles.seatsContainer}>
              ...
              <Text style={styles.seatsText}>
                {isRide
                  ? `${Math.max(0, (item.totalSeats || 0) - (item.availableSeats || 0))}/${item.totalSeats || 0}`
            ...
            <View style={[styles.seatsContainer, { marginLeft: 12 }]}>
              ...
              <Text style={styles.seatsText}>
                {isRide
                  ? `${Math.max(0, (item.luggageCapacityTotal ?? 0) - (item.luggageRemainingTotal ?? item.luggageCapacityTotal ?? 0))}/${item.luggageCapacityTotal ?? 0} bags`
