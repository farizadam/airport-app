...
export default function AvailableRequestsScreen() {
  ...
  const params = useLocalSearchParams<{
    ...
    prefillAirportCode?: string;
    prefillAirportName?: string;
  ...
  // Theme based on direction
  const isToAirport = params.prefillDirection === "to_airport";
  const themeColor = isToAirport ? "#3B82F6" : "#8B5CF6";
  const themeGradient: [string, string] = isToAirport 
    ? ["#3B82F6", "#2563EB"] 
    : ["#8B5CF6", "#7C3AED"];
  
  // Check if we have prefilled search info to display
  const hasSearchInfo = params.autoFilter === "true" && params.prefillAirportId;
  ...
  useEffect(() => {
    ...
    // Cleanup: Reset hasAppliedPrefill when component unmounts
    return () => {
  ...
  useEffect(() => {
    ...
    if (params.autoFilter === "true" && params.prefillAirportId) {
      // Apply prefilled filters automatically
      const newFilters = {
        airport_id: params.prefillAirportId || "",
        direction: params.prefillDirection || "",
      ...
      if (params.prefillDate) {
        setFilterDate(new Date(params.prefillDate));
      }
      
      // Load requests with prefilled filters
      getAvailableRequests(newFilters);
      setHasAppliedPrefill(true);
    } else if (!hasAppliedPrefill) {
      // Normal load without prefill
      loadRequests();
      setHasAppliedPrefill(true);
    }
  }, [params.autoFilter, params.prefillAirportId, params.prefillDirection, params.prefillDate]);
      ...
      {item.max_price_per_seat && (
        <View style={styles.priceRow}>
          ...
          <Text style={styles.priceValue}>
            up to {item.max_price_per_seat} EUR/seat
          </Text>
        </View>
      )}
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
        ...
        >
          ...
          <View style={styles.headerContent}>
            ...
            <Text style={styles.headerSubtitle}>
              {params.prefillAirportCode} • {params.prefillDate ? new Date(params.prefillDate).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "Any date"}
            </Text>
          </View>
          <TouchableOpacity
      ...
      {/* Search summary - only show when we have prefilled info */}
      {hasSearchInfo && (
        <View style={styles.searchSummary}>
          <View style={styles.summaryItem}>
            <Ionicons name="airplane" size={16} color={themeColor} />
            <Text style={styles.summaryText}>{params.prefillAirportName}</Text>
          </View>
          {params.prefillLocationAddress && (
            <View style={styles.summaryItem}>
              <Ionicons name="location" size={16} color={themeColor} />
              <Text style={styles.summaryText} numberOfLines={1}>{params.prefillLocationAddress}</Text>
            </View>
          )}
          {params.prefillDate && (
            <View style={styles.summaryItem}>
              ...
              <Text style={styles.summaryText}>
                {new Date(params.prefillDate).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}
              </Text>
            </View>
          )}
          {params.prefillSeats && (
            <View style={styles.summaryItem}>
              <Ionicons name="people" size={16} color={themeColor} />
              <Text style={styles.summaryText}>{params.prefillSeats} seats available</Text>
            </View>
          )}
        </View>
      )}
      ...
      {loading && availableRequests.length === 0 ? (
        <View style={styles.loadingContainer}>
          <ActivityIndicator size="large" color={hasSearchInfo ? themeColor : "#007AFF"} />
        </View>
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>
            {hasSearchInfo 
              ? "No passengers need rides matching your criteria. You can still offer a ride!"
              : "Try adjusting your filters or check back later"}
          </Text>
          {hasSearchInfo && (
            <TouchableOpacity
              style={[styles.createRideButton, { backgroundColor: themeColor }]}
              onPress={() => router.push({
                pathname: "/(tabs)/rides/create",
                params: {
                  prefillAirportId: params.prefillAirportId,
                  prefillDirection: params.prefillDirection,
                  prefillDate: params.prefillDate,
                  prefillLocationAddress: params.prefillLocationAddress,
                  prefillLocationLat: params.prefillLocationLat,
                  prefillLocationLng: params.prefillLocationLng,
                }
              })}
            >
              <Ionicons name="car" size={20} color="#fff" />
              <Text style={styles.createRideButtonText}>Offer a Ride Anyway</Text>
            </TouchableOpacity>
          )}
        </View>
      ) : (
                  ...
                  <>
                    ...
                    {selectedRequest.max_price_per_seat && (
                      <Text style={styles.budgetHint}>
                        ...
                        {selectedRequest.max_price_per_seat} EUR/seat
                      </Text>
                    )}
