...
import { logTripScreenDisplayed, logTripScreenOpened } from "@/utils/tripCardDebug";
...
export default function RideDetailsScreen() {
  ...
  const [openedLoggedForId, setOpenedLoggedForId] = useState<string | null>(null);
  const [displayedLoggedForId, setDisplayedLoggedForId] = useState<string | null>(null);

  const updateLuggageCount = (count: number) => {
    setLuggageCount(count);
    setLuggage({ sac: count, '10kg': 0, '20kg': 0, 'hors_norme': 0 });
  };

  useEffect(() => {
    if (!id || typeof id !== "string") return;
    if (openedLoggedForId === id) return;
    logTripScreenOpened("ride", id, "RideDetailsScreen");
    setOpenedLoggedForId(id);
    setDisplayedLoggedForId(null);
  }, [id, openedLoggedForId]);

  useEffect(() => {
    if (!id || typeof id !== "string") return;
    if (loading) return;
    if (!ride) return;
    if (displayedLoggedForId === id) return;
    logTripScreenDisplayed("ride", id, "RideDetailsScreen");
    setDisplayedLoggedForId(id);
  }, [id, loading, ride, displayedLoggedForId]);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const loadRideDetails = async () => {
        ...
        const cachedId = (currentRide as any)?._id || (currentRide as any)?.id;
        const isSameRide = cachedId && String(cachedId) === String(id);
        if (isSameRide && !ride) {
          setRide(currentRide as Ride);
        }
        if (!ride && !isSameRide) {
          setLoading(true);
        }
        ...
        try {
          ...
          if (userId && driverId && userId === driverId) {
            ...
            getRideBookings(id).catch((err) => console.log("Ride bookings fetch error:", err));
          }
      ...
      return () => {
        console.log("👋 RideDetailsScreen BLURRED/UNMOUNTED.");
        useBookingStore.getState().clearRideBookings();
      };
    }, [id, user?.id, currentRide, ride])
  );
  ...
  const upsertMyBooking = (booking: any) => {
    if (!booking) return;
    const bookingWithRide = ride
      ? { ...booking, ride, ride_id: booking.ride_id || ride._id || ride.id }
      : booking;
    useBookingStore.setState((state) => {
      const bookingId = bookingWithRide._id || bookingWithRide.id;
      if (!bookingId) return state;
      const exists = state.myBookings.some(
        (b: any) => (b._id || b.id) === bookingId,
      );
      if (exists) return state;
      return { myBookings: [bookingWithRide, ...state.myBookings] };
    });
  };
  ...
  const processWalletPayment = async () => {
    try {
      ...
      if (result.success) {
        if (result.booking) {
          upsertMyBooking(result.booking);
        }
        // Refresh bookings
        await getMyBookings();
  ...
  const processPaymentAndBooking = async () => {
    try {
      ...
      if (completeResponse.data?.booking) {
        upsertMyBooking(completeResponse.data.booking);
      }
      // Refresh bookings
      await getMyBookings();
  ...
  const mapItems = useMemo(() => {
    ...
    if (ride.home_latitude && ride.home_longitude) {
      items.push({
        _id: 'origin',
        type: 'request', // Using 'request' type gets the red marker style
        pickup_location: {
  ...
  const totalSeats =
    ride.seats_total ?? ride.total_seats ?? ride.seats_left ?? ride.available_seats ?? 0;
  const seatsLeft = ride.seats_left ?? ride.available_seats ?? 0;
  const bookedSeats = totalSeats > 0 ? Math.max(0, totalSeats - seatsLeft) : 0;
  const luggageCapacityTotal = (() => {
    const capacity = ride.luggage_capacity as unknown;
    if (typeof capacity === "number") return capacity;
    if (capacity && typeof capacity === "object") {
      return Object.values(capacity as Record<string, number>).reduce(
  ...
  const luggageRemainingTotal = (() => {
    const legacyLeft = (ride as any).luggage_left as number | undefined;
    if (typeof legacyLeft === "number") return legacyLeft;
    const remaining = (ride as any).luggage_remaining as
      | Record<string, number>
      | undefined;
    if (remaining && typeof remaining === "object") {
      return Object.values(remaining).reduce(
        (sum, value) => sum + (Number(value) || 0),
        0
      );
    }
  ...
  const luggageUsedTotal = Math.max(0, luggageCapacityTotal - luggageRemainingTotal);
  const luggageBreakdown = (() => {
    const capacity = ride.luggage_capacity as unknown;
    const remaining = (ride as any).luggage_remaining as Record<string, number> | undefined;
    if (!capacity || typeof capacity !== "object") return "";
    const capacityObj = capacity as Record<string, number>;
    const getValue = (obj: Record<string, number> | undefined, keys: string[]) => {
    ...
    const buildEntry = (
      ...
      const total = getValue(capacityObj, capacityKeys);
      if (!total || total <= 0) return "";
      const left = getValue(remaining, remainingKeys);
      const used = typeof left === "number" ? Math.max(0, total - left) : 0;
      return `${label} ${used}/${total}`;
    };

    const line1 = [
      buildEntry("🧳 10kg", ["max_10kg", "10kg", "count_10kg"], ["count_10kg", "10kg"]),
      buildEntry("💼 20kg", ["max_20kg", "20kg", "count_20kg"], ["count_20kg", "20kg"]),
    ]
      .filter(Boolean)
      .join(" | ");
    const line2 = [
      buildEntry("🎒 sac", ["max_sac", "sac", "count_sac"], ["count_sac", "sac"]),
      buildEntry("📦 hors norme", ["max_hors_norme", "hors_norme", "count_hors_norme"], ["count_hors_norme", "hors_norme"]),
    ]
      .filter(Boolean)
      .join(" | ");
          ...
          <View style={styles.detailsGrid}>
            <View style={styles.detailItem}>
              ...
              <Text style={styles.detailValue}>
                {totalSeats > 0 ? `${bookedSeats}/${totalSeats}` : "--"}
              </Text>
            </View>
            <View style={styles.detailItem}>
              ...
              <Text style={styles.detailValue}>
                {luggageCapacityTotal > 0
                  ? `${luggageUsedTotal}/${luggageCapacityTotal}`
                  : "0/0"}
              </Text>
              {luggageBreakdown.length > 0 && (
                <Text style={styles.detailSubValue}>{luggageBreakdown}</Text>
              )}
            </View>
            <View style={styles.detailItem}>
            ...
            <View style={styles.modalContent}>
              ...
              <View style={styles.modalInfo}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
                  <Text style={styles.modalInfoText}>
                    Seats: {totalSeats > 0 ? `${bookedSeats}/${totalSeats}` : "--"}
                  </Text>
                  <Text style={styles.modalInfoText}>
                    Luggage: {luggageCapacityTotal > 0
                      ? `${luggageUsedTotal}/${luggageCapacityTotal}`
                      : "0/0"}
                  </Text>
                </View>
              ...
              <View style={styles.inputContainer}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 16, marginTop: 4 }}>
                  ...
                  <TouchableOpacity
                    onPress={() => setSeats(Math.min(seatsLeft, seats + 1))}
                    style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: seats < seatsLeft ? '#DBEAFE' : '#F1F5F9', alignItems: 'center', justifyContent: 'center' }}
                    disabled={seats >= seatsLeft}
                  >
                    <Ionicons name="add" size={22} color={seats < seatsLeft ? '#3B82F6' : '#CBD5E1'} />
                  </TouchableOpacity>
                </View>
              </View>
              ...
              <View style={styles.inputContainer}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 16, marginTop: 4 }}>
                  <TouchableOpacity
                    onPress={() => updateLuggageCount(Math.max(0, luggageCount - 1))}
                  ...
                  <TouchableOpacity
                    onPress={() => updateLuggageCount(Math.min(luggageRemainingTotal, luggageCount + 1))}
                    style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: luggageCount < luggageRemainingTotal ? '#DBEAFE' : '#F1F5F9', alignItems: 'center', justifyContent: 'center' }}
                    disabled={luggageCount >= luggageRemainingTotal}
                  >
                    <Ionicons name="add" size={22} color={luggageCount < luggageRemainingTotal ? '#3B82F6' : '#CBD5E1'} />
                  </TouchableOpacity>
                </View>
              </View>
              ...
              <View style={styles.paymentMethodSection}>
                <Text style={styles.paymentMethodTitle}>Payment Method ssss</Text>
