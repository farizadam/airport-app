...
import { startTripCardNavigationTrace } from "../utils/tripCardDebug";
...
export const TripCard = ({ item, onCancel, showCancelButton = false }: TripCardProps) => {
  ...
  const getLuggageDisplay = () => {
    const capacity = item.luggage_capacity as unknown;
    const totalCapacity =
      typeof capacity === "number"
        ? capacity
        : capacity && typeof capacity === "object"
          ? Object.values(capacity as Record<string, number>).reduce(
              (sum, value) => sum + (Number(value) || 0),
              0
            )
          : undefined;
    const remaining = (() => {
      if (typeof item.luggage_left === "number") return item.luggage_left;
      const remainingObj = (item as any).luggage_remaining as
        | Record<string, number>
        | undefined;
      if (remainingObj && typeof remainingObj === "object") {
        return Object.values(remainingObj).reduce(
          (sum, value) => sum + (Number(value) || 0),
          0
        );
      }
    ...
    if (totalCapacity !== undefined) {
      if (typeof remaining === "number") {
        return `${Math.max(0, totalCapacity - remaining)}/${totalCapacity} bags`;
      }
      return `${totalCapacity} bags`;
    }

    const luggageWithQty = (item.luggage || []).filter((l) => (l.quantity || 0) > 0);
    if (luggageWithQty.length > 0) {
      const totalQty = luggageWithQty.reduce((sum, l) => sum + (l.quantity || 0), 0);
      return `${luggageWithQty.length} type${luggageWithQty.length !== 1 ? "s" : ""} (${totalQty} bag${totalQty !== 1 ? "s" : ""})`;
    }

    if (item.luggage_count !== undefined) {
      return `${item.luggage_count} bag(s)`;
    }
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() => {
        ...
        if (item.type === "ride") {
          startTripCardNavigationTrace("ride", item.id, "TripCard:ride");
          useRideStore.getState().fetchRideById(item.id).catch(() => { });
          router.push({ pathname: "/ride-details/[id]", params: { id: item.id } });
        } else if (item.type === "request") {
          startTripCardNavigationTrace("request", item.id, "TripCard:request");
          useRequestStore.getState().fetchRequestById(item.id).catch(() => { });
          router.push({ pathname: "/request-details/[id]", params: { id: item.id } });
        } else if (item.type === "booking") {
          if (item.rideId) {
            startTripCardNavigationTrace("ride", item.rideId, "TripCard:booking");
            useRideStore.getState().fetchRideById(item.rideId).catch(() => { });
          ...
          if (item.requestId) {
            startTripCardNavigationTrace("request", item.requestId, "TripCard:offer");
            useRequestStore.getState().fetchRequestById(item.requestId).catch(() => { });
      ...
      <View style={styles.tripFooter}>
        <View style={styles.tripInfoLeft}>
          ...
          <View style={[styles.tripSeats, { marginLeft: 10 }]}>
            ...
            <Text style={styles.tripSeatsText}>{getLuggageDisplay()}</Text>
          </View>
        </View>

        {/* Cancel Button (Only for drivers — passengers cannot cancel from card) */}
        {showCancelButton && onCancel && item.role !== "passenger" && item.type !== "offer" && (
          <TouchableOpacity
