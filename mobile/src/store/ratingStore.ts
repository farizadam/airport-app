...
export const useRatingStore = create<RatingState>((set, get) => ({
  ...
  fetchMyRatings: async () => {
    ...
    try {
      const response = await apiGet("/ratings/me", undefined, { ttlMs: 12_000 });
  ...
  fetchPendingRatings: async () => {
    set({ isLoading: true, error: null });
    try {
      const response = await apiGet("/ratings/pending", undefined, { ttlMs: 12_000 });
      if (response.data.success) {
  ...
  fetchUserRatings: async (userId: string) => {
    set({ isLoading: true, error: null });
    try {
      const response = await apiGet(`/ratings/user/${userId}`, undefined, { ttlMs: 12_000 });
      if (response.data.success) {
  ...
  fetchUserRatingStats: async (userId: string) => {
    set({ isLoading: true, error: null });
    try {
      const response = await apiGet(`/ratings/stats/${userId}`, undefined, { ttlMs: 20_000 });
      if (response.data.success) {
  ...
  checkCanRate: async (bookingId: string) => {
    try {
      const response = await apiGet(`/ratings/can-rate/${bookingId}`, undefined, { ttlMs: 8_000 });
      if (response.data.success) {
        return response.data.data;
      }
      return null;
