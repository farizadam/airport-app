...
export const useWalletStore = create<WalletState>((set, get) => ({
    ...
    try {
      set({ isLoading: true, error: null });
        ...
        pagination: {
          ...
          }>(`/wallet/payouts?page=${page}&limit=20`, undefined, { ttlMs: 8_000 });
    ...
    try {
      set({ isLoading: true, error: null });
        ...
        data: EarningsSummary;
          }>("/wallet/earnings-summary", undefined, { ttlMs: 15_000 });
  ...
  getBankStatus: async () => {
    try {
      set({ isLoading: true, error: null });
        ...
        success: boolean;
        data: BankStatus;
          }>("/wallet/bank-status", undefined, { ttlMs: 10_000 });
  ...
  requestWithdrawal: async (amount: number) => {
    try {
      ...
      // Ensure UI reflects restored balance from backend rollbacks
      try {
        clearApiGetCache("/wallet");
        await get().getWallet();
      } catch (refreshErr) {
        console.warn("Failed to refresh wallet after withdrawal error", refreshErr);
      }
  ...
  setupPayouts: async () => {
    try {
      ...
      const response = await apiGet<{
        success: boolean;
        url: string;
      }>("/payments/onboarding-link", undefined, { ttlMs: 15_000 });
  ...
  calculateEarnings: async (pricePerSeat: number, seats: number) => {
    try {
      ...
      }>(`/wallet/calculate-earnings?price_per_seat=${pricePerSeat}&seats=${seats}`, undefined, { ttlMs: 10_000 });
  ...
  payWithWallet: async (rideId: string, seats: number, luggage: { type: string; quantity: number }[]) => {
    try {
      ...
      }>("/payments/wallet", { rideId, seats, luggage: luggage || [] });
      clearApiGetCache("/wallet");
