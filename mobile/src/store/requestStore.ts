...
export const useRequestStore = create<RequestStore>((set, get) => ({
  ...
  createRequest: async (data) => {
    ...
    try {
      ...
      clearApiGetCache("/ride-requests");
  ...
  getMyRequests: async (status?: string) => {
    ...
    try {
      ...
      const response = await apiGet(`/ride-requests/my-requests?${params}`, undefined, { ttlMs: 12_000 });
      set({
  ...
  getAvailableRequests: async (filters?: any) => {
    ...
    try {
      ...
      const response = await apiGet(`/ride-requests/available?${params}`, undefined, { ttlMs: 10_000 });
  ...
  getMyOffers: async (status?: string) => {
    ...
    try {
      ...
      const response = await apiGet(`/ride-requests/my-offers?${params}`, undefined, { ttlMs: 12_000 });
      set({
  ...
  getRequest: async (requestId: string) => {
    set({ loading: true, error: null });
    try {
      const response = await apiGet(`/ride-requests/${requestId}`, undefined, { ttlMs: 8_000 });
      set({
        currentRequest: response.data.request,
        loading: false,
      });
  ...
  fetchRequestById: async (requestId: string) => {
    try {
      const response = await apiGet(`/ride-requests/${requestId}`, undefined, { ttlMs: 8_000 });
      set({ currentRequest: response.data.request });
