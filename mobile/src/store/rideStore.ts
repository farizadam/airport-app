...
export const useRideStore = create<RideState>((set) => ({
  ...
  searchRides: async (filters) => {
    try {
      ...
      const response = await apiGet<PaginatedResponse<Ride>>(
        `/rides/search?${params}`,
        undefined,
        { ttlMs: 15_000 }
      );
  ...
  getRideById: async (id: string) => {
    ...
    try {
      ...
      const response = await apiGet(`/rides/${id}`, undefined, { ttlMs: 8_000 });
  ...
  getMyRides: async () => {
    try {
      ...
      set({ isLoading: true });
      const response = await apiGet("/rides/my-rides", undefined, { ttlMs: 12_000 });
      const fetchedCount = response.data.data?.length || 0;
      console.log("✅ My rides fetched:", fetchedCount, "rides");
